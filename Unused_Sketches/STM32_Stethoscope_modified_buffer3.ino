...
uint8_t ADC1_Sequence[] __attribute__ ((aligned(1))) = {0};//,2,3,4,5,6,7};
...
dma_tube_config my_tube_cfg = {
	...
	DMA_SIZE_16BITS,	// source transfer size || DMA_SIZE_32BITS
	&adc_buffer,		// data destination address 
	DMA_SIZE_16BITS,	// destination transfer size  || DMA_SIZE_32BITS
	TOTAL_DATA,	// nr. of data to transfer  || SAMPLES_PER_BUFFER
...
void loop()
{
	...
	for(block_nr=0; block_nr<2; ) {
        ...
        if (!sd.card()->writeData((const uint8_t*)adc_buffer)) {
        error("writeData failed");
        }
        block_nr ++;
        break;
